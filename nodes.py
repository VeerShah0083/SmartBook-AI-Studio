...
def _llm(system: str, user: str, *, max_tokens: int = 3500,
          temperature: float = 0.6) -> str:
    """Thin wrapper: call Groq and return the text response."""
    import groq
    import time
    ...
    for attempt in range(5):
        try:
            ...
            if e.status_code == 413 or "rate_limit_exceeded" in str(e):
                if attempt == 4:
                    raise
                print(f"    [Rate Limit Hit] Waiting 60 seconds before retrying... (Attempt {attempt+1}/5)")
                time.sleep(60)
            else:
                raise
...
def _llm_json(system: str, user: str, *, max_tokens: int = 3500,
    ...
    # Retry up to 3 times if the LLM output is truncated or invalid
    for attempt in range(3):
        raw = _llm(system + suffix, user, max_tokens=max_tokens, temperature=temperature)
        text = raw.strip()
        
        # Remove <think>...</think> blocks, even if unclosed
        text = re.sub(r"<think>.*?(?:</think>|$)", "", text, flags=re.DOTALL).strip()
        
        # Extract json block if enclosed in markdown formatting
        if "```" in text:
        ...
        # If we failed to parse, wait a moment and try again
        import time
        time.sleep(1)

    raise ValueError(f"Could not parse JSON after 3 attempts.\nLast raw response:\n{raw[:600]}")
...
Based on the subject and grade level, produce a
...
CRITICAL INSTRUCTION: To make the textbook highly engaging, you MUST plan to incorporate real-world story excerpts, analogies, and historical examples wherever applicable. For instance, if the topic is character building, plan to include stories of real people like Steve Jobs, Malala Yousafzai, etc. Ensure the curriculum relies heavily on relatable narratives.
...
6. Pedagogical philosophy (Focus on storytelling, experiential learning, and real-world application)
...
def scope_architect(state: TextbookSystemState) -> dict:
    ...
    # ── INTERRUPT 2: export format preference ──────────────────────────────
    ...
    pedagogy  = "Story-driven, experiential, and narrative-focused"
    standards = []
